...
public class Deliver extends AppCompatActivity {
    ...
    public void buttonFind(View view) {
        ...
        EditText yearET = findViewById(R.id.year);
        if (yearET.hasFocus()) {
            toast("Select ReceiptNo, Name or MobileNo");
            return;
        }
        mtempArray.clear();
        salesArray.clear();
        ...
        int yearInt = UHelper.parseInt(year);

        if (yearInt < 2000 || yearInt > 2999)
            year = UHelper.getTime("y");
        ...
        if (receiptNo.hasFocus() || receiptNoTo.hasFocus()) {
            ...
            if (rno > 0 && rnoTo == 0) {
                ...
                q = mDatabase.child(UHelper.getTime(year)).orderByChild("receiptNo").equalTo(no);
            }
            if (rno > 0 && rnoTo > 0) {
                String from = String.format(Locale.US, "%04d", rno);
                String to = String.format(Locale.US, "%04d", rnoTo);
                q = mDatabase.child(UHelper.getTime(year)).orderByChild("receiptNo").startAt(from).endAt(to);
            }
            if (rno == 0 && rlength > 0) {
                q = mDatabase.child(UHelper.getTime(year)).orderByChild("receiptNo").startAt(receiptNo.getText().toString()).endAt(receiptNo.getText().toString() + "\uf8ff");
            }

        } else if (mobileno.hasFocus()) {
            if (mobileno.getText().toString().length() != 0)
                q = mDatabase.child(year).orderByChild("mobile").startAt(mobileno.getText().toString()).endAt(mobileno.getText().toString() + "\uf8ff");
        } else if (name.hasFocus()) {
            if (name.getText().toString().length() != 0)
                q = mDatabase.child(year).orderByChild("name").startAt(name.getText().toString().toLowerCase()).endAt(name.getText().toString().toLowerCase() + "\uf8ff");
        }
    ...
    void updateEntry(final Query finalQ) {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            if (dataSnapshot.exists()) {
                                for (DataSnapshot data : dataSnapshot.getChildren()) {
                                    ...
                                    salesArray.add(temp);
                                    ClayModel mTemp = new ClayModel(temp.getReceiptNo(), temp.getName(), "R.No : " + temp.getReceiptNo() + "   Year : " + temp.getDate().substring(0, 4) + "\nMob  : " + temp.getMobile());
    ...
    void showInputDialog(final SellModel temp) {
        ...
        final ImageView call = promptView.findViewById(R.id.callImage);
...
/*        for (int i = 0; i < modelList.size(); i++) {
        ...
        call.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                dialIntent.setData(Uri.parse("tel:" + temp.getMobile()));
        ...
        String dt = salesArray.get(transactionID).getDate();
